...
import type { CourseFullResponse, Review, BonusItem } from '../../api/types';
...
const CoursePage = () => {
  ...
  // Обработка клика по кнопке действия (Купить / Начать)
  const handleActionClick = () => {
    ...
    if (isFree || isPurchased) {
      ...
      // Логика покупки (Заглушка для ЮKassa)
      handlePurchase();
    }
  };
  ...
  const handlePurchase = () => {
    ...
    const confirmBuy = window.confirm(
      `Вы собираетесь приобрести курс "${courseData.course.title}" за ${courseData.course.price} ₽.\n\n(Здесь будет открытие формы оплаты ЮKassa)`
    );

    if (confirmBuy) {
      // TODO: Здесь будет вызов API создания платежа ЮKassa
      // const paymentUrl = await createPayment(courseId);
      // window.location.href = paymentUrl;

      alert('Переход к платежному шлюзу... (Интеграция в разработке)');
    }
  };
  ...
  // Вспомогательная функция для рендеринга списков из строки с разделителем |||
  ...
  const renderList = (text: string, type: 'bad' | 'good') => {
    ...
    // 1. Заменяем точки с пробелом на специальный разделитель, чтобы разбить предложения
    // 2. Также заменяем переносы строк на тот же разделитель (для обратной совместимости)
    const normalizedText = text.replace(/\. /g, '|||').replace(/\n/g, '|||');

    // Разбиваем по разделителю
    const items = normalizedText.split('|||')
      .map(item => item.trim()) // Убираем пробела по краям
      .filter(item => item.length > 0); // Убираем пустые
    ...
    return (
      <section className="bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-gray-100 mb-8">
        ...
        <div className="space-y-3">
          {items.map((line, idx) => {
            // Очищаем строку от лишних эмодзи в начале, если они остались после сплита
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="order-2 lg:order-1">
              ...
              <button
                ...
                className={`w-full sm:w-auto px-10 py-4 text-lg font-bold text-white rounded-2xl! shadow-lg transition-all transform hover:-translate-y-1 hover:shadow-xl ${buttonProps.color}`}
              >
                {buttonProps.text}
              </button>
            </div>
      ...
      <div className="max-w-250 mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
          ...
          <section>
            ...
            <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
              ...
              <div className="mt-10 flex justify-center">
                <button
                  onClick={buttonProps.action}
                  className={`px-8 py-3 text-base font-bold text-white rounded-2xl! shadow-md transition-transform hover:-translate-y-0.5 ${buttonProps.color}`}
                >
                  {buttonProps.text}
                </button>
              </div>
        ...
        {/* 3. КУРС ВКЛЮЧАЕТ В СЕБЯ (Course Basis) */}
        {course.course_basis && (
          <section>
            <h2 className="text-3xl font-serif font-bold text-gray-900 mb-8! text-center">
        ...
        {course.bonuses && course.bonuses.length > 0 && (
          <section className="pt-4">
            <h3 className="text-2xl font-serif font-bold text-gray-900 mb-10! text-center">
              🎁 Бонусы при покупке
            </h3>
        ...
        <section className="pt-8 border-t border-gray-200">
          ...
          <Comments courseId={courseId} reviews={reviews} onReviewSubmitted={() => window.location.reload()} />
        </section>
